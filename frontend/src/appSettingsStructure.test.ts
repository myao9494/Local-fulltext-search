...
test("検索時は入力パスが空なら保存済みの既定検索フォルダへフォールバックする", () => {
  ...
  assert.match(source, /const resolvedSearchPath = normalizedInputPath \|\| \(isSearchAllEnabled \? "" : savedDefaultSearchPath\);/);
...
/**
 * 全 DB 検索中に入力欄が空なら、既定フォルダへはフォールバックせず全階層を検索する。
 */
test("全データベース検索かつフォルダ未指定なら既定フォルダへフォールバックしない", () => {
  const source = readFileSync(appPath, "utf-8");

  assert.match(source, /const resolvedSearchPath = normalizedInputPath \|\| \(isSearchAllEnabled \? "" : savedDefaultSearchPath\);/);
  assert.doesNotMatch(source, /const resolvedSearchPath = fullPath\.trim\(\) \|\| savedDefaultSearchPath;/);
});
