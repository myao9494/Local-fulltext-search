...
/**
 * ファイル検索結果には、以後の検索から除外する「無視」ボタンを表示する。
 */
test("検索結果には無視ボタンと無視ハンドラを含める", () => {
  const source = readFileSync(resultsListPath, "utf-8");
  const styleSource = readFileSync(appStylesPath, "utf-8");

  assert.match(source, /onResultIgnore/);
  assert.match(source, /className="result-ignore-button"/);
  assert.match(source, /無視/);
  assert.match(styleSource, /\.result-ignore-button\s*\{/);
});
