...
### source_type + path 用インデックス

`files(source_type, normalized_path)` と `targets(is_search_target_enabled, source_type, full_path)` の補助インデックスを追加し、  
ローカル/Web 切り替えとパス範囲条件を同時に使う検索・再インデックス判定を安定して高速化する。

- `files.source_type = ?` と `normalized_path` 範囲条件の組み合わせを高速化
- 有効な検索対象フォルダの親判定・検索対象一覧取得を高速化
- ローカル検索時に Web レコードが増えても劣化しにくい

### 既存メタデータの再利用

再インデックス時に先に読み込んだ `existing_files` を削除判定にも再利用し、  
同じ対象ルート配下の `files` を二重に読み直さないようにした。

- 変更有無の差分判定
- 削除済みファイルの検出
- 大きい対象フォルダでも DB 往復を 1 回分削減
