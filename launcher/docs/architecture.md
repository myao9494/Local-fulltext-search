...
### ランチャーの Python 環境
ランチャー子プロセスはプロジェクトルート `.venv` の Python を優先し、存在しない場合はバックエンド実行中の Python へフォールバックします。自動起動を使う環境では、選ばれる Python 環境に `launcher/requirements.txt` の依存関係をインストールしておきます。
