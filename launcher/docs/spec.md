...
- Windows / Linux の Flet 版では、再表示時に `center`、`to_front`、検索欄 `focus` を順に実行し、検索結果の再描画後も検索欄へフォーカスを戻す。Windows では検索欄フォーカス中の単独 `Enter` を IME 変換確定として扱い、検索結果起動には使わない。Flet 側の Enter イベントが失われた場合に備え、ランチャー表示中かつ検索画面または gantt メモ画面の単独 `Enter` は `pynput` のグローバルキー監視でも補助的に拾うが、Windows の検索欄フォーカス中は横取りしない。
